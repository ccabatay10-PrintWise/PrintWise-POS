  ...
  const loadDashboard = async () => {
    ...
    try {
      ...
      setData({
        ...
        lowStock: lowStockAll.slice(0, 4),
        recentOrders,
        trend,
        statusCounts,
      });
      ...
      <section className="workspace dashboard-workspace dashboard-v2">
        ...
        <div className="dashboard-stats dashboard-kpis">
          <article className="dash-stat primary-stat">
            ...
            <div>
              ...
              <small><CheckCircle2 size={14} /> All completed transactions</small>
            </div>
          </article>
          ...
          <article className="dash-stat">
            ...
            <div>
              ...
              <small>Completed sales in selected period</small>
            </div>
          </article>
          ...
          <article className="dash-stat">
            ...
            <div>
              ...
              <small>{data.totalOrders} total orders in selected period</small>
            </div>
          </article>
        ...
        <div className="dashboard-analytics-grid">
          <section className="dashboard-card sales-trend-card">
            <div className="card-title">
              <div>
                <h2>Sales Trend</h2>
                <p>Completed sales for {periodLabel.toLowerCase()}.</p>
              </div>
            ...
            <div className="sales-bars">
              {data.trend.map((item) => (
                <div className="sales-bar-item" key={item.key}>
                  <div className="sales-bar-track">
                    <div
                      className="sales-bar-fill"
                      style={{
                        height: `${Math.max((item.amount / maxTrend) * 100, item.amount > 0 ? 8 : 2)}%`,
                      }}
          ...
          <section className="dashboard-card order-health-card">
            <div className="card-title">
              <div>
                <h2>Order Health</h2>
                <p>{periodLabel} order activity.</p>
              </div>
            ...
            <div className="average-order">
              <span>Average Completed Order ({periodLabel})</span>
        ...
        <div className="dashboard-main-grid">
          <section className="dashboard-card recent-card">
            <div className="card-title">
              <div>
                ...
                <p>Latest 5 transactions from your PrintWise POS.</p>
              </div>
              <a href="/orders">VIEW ALL <ArrowRight size={16} /></a>
            </div>
            ...
            <div className="transaction-list">
              {loading ? (
                ...
                <div className="dashboard-empty">No transactions yet. Start selling from the Point of Sale.</div>
              ) : (
                data.recentOrders.map((order) => (
                  <div className="transaction" key={order.id}>
                    ...
                    <span className={`transaction-status status-${order.status.replace(/\s+/g, "-")}`}>
                      {order.status}
                    </span>
                  </div>
                ))
          ...
          <section className="dashboard-card inventory-card">
            <div className="card-title">
              ...
              <a href="/inventory">MANAGE <ArrowRight size={16} /></a>
            </div>

            <div className="inventory-summary">
              <div><Boxes size={20} /><span>Active Products</span><b>{data.productCount}</b></div>
              <div><AlertTriangle size={20} /><span>Low Stock</span><b>{data.lowStockCount}</b></div>
            </div>
            ...
            <div className="low-stock-list">
              {loading ? (
                ...
                <div className="stock-good">✓ All tracked inventory is above the reorder level.</div>
              ) : (
                data.lowStock.map((item) => (
                  <div className="low-stock-item" key={item.id}>
                    <div><b>{item.name}</b><span>{item.category}</span></div>
                    <strong>{item.quantity} {item.unit}</strong>
                  </div>
                ))
              )}
              {!loading && data.lowStockCount > data.lowStock.length && (
                <div className="dashboard-empty">
                  +{data.lowStockCount - data.lowStock.length} more low-stock item(s)
                </div>
              )}
            </div>
          </section>
        ...
        <div className="dashboard-bottom-grid quick-dashboard-actions">
          <a className="action-card" href="/pos">
            <div className="action-icon"><ShoppingCart size={21} /></div>
            <div><b>Start a New Sale</b><span>Open Point of Sale</span></div>
            <ArrowRight size={18} />
          </a>
          <a className="action-card" href="/orders">
            <div className="action-icon"><ReceiptText size={21} /></div>
            <div><b>Review Orders</b><span>View and manage transactions</span></div>
            <ArrowRight size={18} />
          </a>
          <a className="action-card" href="/gcash-bayad">
            <div className="action-icon"><CreditCard size={21} /></div>
            <div><b>GCash / Bayad</b><span>Review payment transactions</span></div>
            <ArrowRight size={18} />
          </a>
        </div>
      </section>
