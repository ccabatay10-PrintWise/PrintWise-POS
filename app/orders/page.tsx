...
import { ArrowLeft, ReceiptText, Search, User, CalendarDays, CreditCard } from "lucide-react";
...
export default function OrdersPage() {
  ...
  const filtered = orders.filter(order =>
    `${order.order_no} ${order.customer_name || ""} ${order.status}`.toLowerCase().includes(search.toLowerCase())
  );
        ...
        <section className="catalog-panel">
          ...
          <div style={{ overflowX: "auto", marginTop: 18 }}>
            <table className="orders-table">
              ...
              <tbody>{loading ? <tr><td colSpan={6}>Loading orders...</td></tr> : filtered.length === 0 ? <tr><td colSpan={6}>No orders found.</td></tr> : filtered.map(order => <tr key={order.id}>
                <td><b>{order.order_no}</b></td><td><User size={14} /> {order.customer_name || "Walk-in Customer"}</td>
                <td><CalendarDays size={14} /> {new Date(order.created_at).toLocaleString()}</td>
                <td>₱{order.total.toFixed(2)}</td><td>₱{order.amount_paid.toFixed(2)}</td>
