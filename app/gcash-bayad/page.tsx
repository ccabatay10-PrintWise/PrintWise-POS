...
const getServiceFee = (value: number) => {
  if (!Number.isFinite(value) || value <= 0) return 0;
  return feeBrackets.find((bracket) => value >= bracket.min && value <= bracket.max)?.fee ?? 100;
};

const getFeeLabel = (value: number) => {
  if (!Number.isFinite(value) || value <= 0) return "Enter an amount to calculate the fee";
  const bracket = feeBrackets.find((item) => value >= item.min && value <= item.max);
  return bracket
    ? `Automatic fee for ₱${bracket.min.toLocaleString()}–₱${bracket.max.toLocaleString()}`
    : "₱5,001+ currently uses the maximum ₱100 fee";
};
...
export default function GCashBayadPage() {
  ...
  const automaticFee = getServiceFee(amountNumber);
  const customerTotal = amountNumber > 0 ? amountNumber + automaticFee : 0;
  ...
  const handleSave = async (event: FormEvent) => {
    ...
    if (error) {
      ...
      setMessage(`${transactionLabel(type)} saved with an automatic ${peso(automaticFee)} service fee.`);
    ...
    <main className="app-shell">
      ...
      <section className="workspace gcash-workspace">
        <header className="topbar gcash-header">
          <div><div className="eyebrow">SERVICE CENTER</div><h1>GCash / Bayad Services</h1><p>Process and record cash-in and cash-out services with automatic service-fee computation.</p></div>
        ...
        <div className="gcash-content">
          <section className="notice-card"><Clock3 size={20} /><div><strong>Automatic service-fee schedule</strong><span>Fees are calculated automatically using your ₱10-per-₱500 schedule, from ₱10 for ₱1–₱500 up to ₱100 for ₱4,501–₱5,000. The same schedule applies to Cash In and Cash Out.</span></div></section>
          ...
          <section className="metric-grid gcash-metrics">
            ...
            <article className="metric-card"><div className="metric-icon soft-amber"><Banknote size={21} /></div><div className="metric-copy"><span>Service Fees Today</span><strong>{peso(todayFees)}</strong><small>Automatic fees collected</small></div></article>
          ...
          <section className="service-grid">
            <form className="transaction-form-card" onSubmit={handleSave}>
              <div className="form-heading"><div><div className="eyebrow">NEW SERVICE</div><h2>{type === "cash_in" ? "Process Cash In" : "Process Cash Out"}</h2><p>Enter the amount and PrintWise will compute the service fee automatically.</p></div><div className={`service-icon ${type === "cash_in" ? "cash-in" : "cash-out"}`}>{type === "cash_in" ? <ArrowDownLeft size={24} /> : <ArrowUpRight size={24} />}</div></div>
              ...
              <div className="form-grid">
                ...
                <label>Automatic Service Fee<div className="money-input fee-locked"><span>₱</span><input value={automaticFee ? automaticFee.toFixed(2) : "0.00"} readOnly aria-label="Automatic service fee" /></div><small className="fee-help">{getFeeLabel(amountNumber)}</small></label>
              ...
              <div className="amount-preview">
                <div><span>{type === "cash_in" ? "Cash to Receive from Customer" : "Customer Total to Collect"}</span><strong>{peso(customerTotal)}</strong></div>
                <div className="preview-detail"><small>Transaction</small><b>{peso(amountNumber)}</b><small>Service Fee</small><b>{peso(automaticFee)}</b></div>
                {type === "cash_out" && <small className="release-note">Cash to release to customer: <b>{peso(amountNumber)}</b></small>}
              </div>

              <div className="fee-schedule"><div className="fee-schedule-head"><strong>Automatic Fee Schedule</strong><span>Cash In / Cash Out</span></div><div className="fee-grid">{feeBrackets.map((bracket) => <div key={bracket.min} className={amountNumber >= bracket.min && amountNumber <= bracket.max ? "active" : ""}><span>₱{bracket.min.toLocaleString()}–₱{bracket.max.toLocaleString()}</span><b>₱{bracket.fee}</b></div>)}</div></div>
            ...
            <aside className="workflow-card">
              ...
              <ol className="workflow-list"><li><span>1</span><div><strong>Enter the amount</strong><p>The service fee is calculated automatically from the fee schedule.</p></div></li><li><span>2</span><div><strong>Confirm customer details</strong><p>Verify the name and GCash mobile number before processing.</p></div></li><li><span>3</span><div><strong>Collect the total</strong><p>Collect the transaction amount plus the automatic service fee.</p></div></li><li><span>4</span><div><strong>Complete and save reference</strong><p>Mark successful only after the actual wallet transaction is confirmed.</p></div></li></ol>
          ...
          <section className="transactions-card gcash-history">
            <div className="transactions-head"><div><h2>GCash Transaction History</h2><p>{filtered.length} transaction{filtered.length === 1 ? "" : "s"} shown</p></div><div className="history-tools"><div className="search-box payment-search"><Search size={19} /><input value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Search name, mobile, reference..." /></div><select value={statusFilter} onChange={(e) => setStatusFilter(e.target.value)}><option value="all">All Statuses</option><option value="pending">Pending</option><option value="successful">Successful</option><option value="cancelled">Cancelled</option><option value="failed">Failed</option></select></div></div>
            <div className="table-wrap"><table className="payment-table gcash-table"><thead><tr><th>Transaction</th><th>Customer</th><th>Service</th><th>Amount</th><th>Fee</th><th>Reference</th><th>Status</th><th>Action</th></tr></thead><tbody>
