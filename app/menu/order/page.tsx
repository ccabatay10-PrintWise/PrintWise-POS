...
import "./menu-viewport-fixes.css";
...
export default function WiseMenuOrder() {
  ...
  useEffect(()=>{
    const setViewportHeight=()=>document.documentElement.style.setProperty("--wm-visual-vh",`${window.visualViewport?.height||window.innerHeight}px`);
    setViewportHeight();
    window.visualViewport?.addEventListener("resize",setViewportHeight);
    window.visualViewport?.addEventListener("scroll",setViewportHeight);
    return()=>{window.visualViewport?.removeEventListener("resize",setViewportHeight);window.visualViewport?.removeEventListener("scroll",setViewportHeight)};
  },[]);
  ...
  const add=(p:Product,quantity=1,openOrder=false)=>{
    setSubmitError("");
    setCart(c=>{const x=c.find(i=>i.product.id===p.id);return x?c.map(i=>i.product.id===p.id?{...i,quantity:i.quantity+quantity}:i):[...c,{product:p,quantity}]});
    setProductOpen(null);
    setSelectedQty(1);
    if(openOrder){window.requestAnimationFrame(()=>setCartOpen(true));}
  };
  ...
  return <main className="wm-page">
    ...
    {typeof document!=="undefined"&&createPortal(productModal,document.body)}
