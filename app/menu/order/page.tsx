...
import { Check, ChevronLeft, ChevronRight, Mail, Minus, Plus, Search, ShoppingBag, UserRound, Utensils, X } from "lucide-react";
import "./menu.css";
...
export default function WiseMenuOrder() {
  ...
  const continueToMenu=()=>{
    ...
    sessionStorage.setItem("wise-menu-customer",JSON.stringify({name:n,email:e}));
    setName(n);setEmail(e);setProfileError("");setProfileReady(true);
  };
  const change=(id:string,d:number)=>setCart(c=>c.map(i=>i.product.id===id?{...i,quantity:i.quantity+d}:i).filter(i=>i.quantity>0));
  const add=(p:Product)=>{setSubmitError("");setCart(c=>{const x=c.find(i=>i.product.id===p.id);return x?c.map(i=>i.product.id===p.id?{...i,quantity:i.quantity+1}:i):[...c,{product:p,quantity:1}]});setProductOpen(null)};
  ...
  async function submit(){
    ...
    try{
      ...
      sessionStorage.setItem("wise-menu-pending-order",JSON.stringify(pending));
      setPendingOrder(pending);setSuccess(x);setCart([]);setNotes("");setCartOpen(false);
  ...
  const [orderType,setOrderType]=useState<"dine_in"|"take_out">("take_out");
  ...
  return <main className="wm-page"><header className="wm-header"><div className="wm-brand-block"><button className="wm-mobile-menu" type="button" aria-label="Menu"><span/><span/><span/></button><div><div className="wm-logo">WISE <span>MENU</span></div><small>Hi, {name}! What would you like today?</small></div></div><div className="wm-header-actions">{pendingOrder&&<button type="button" className="wm-pending-btn" onClick={()=>setSuccess({order_no:pendingOrder.order_no,total:pendingOrder.total})}>Pending Order <span>1</span></button>}<button type="button" className="wm-cart" onClick={()=>{setSubmitError("");setCartOpen(true)}}><ShoppingBag size={21}/><span>{count}</span></button></div></header>
    ...
    <div className="wm-layout"><aside className="wm-category-sidebar"><div className="wm-category-title">MENU</div>{categories.map((c,i)=><button type="button" className={category===c?"active":""} key={c} onClick={()=>setCategory(c)}><span className="wm-cat-icon">{i===0?<Utensils size={17}/>:<span>{c.charAt(0)}</span>}</span><b>{c}</b></button>)}</aside><section className="wm-content"><div className="wm-content-head"><div><span className="wm-eyebrow">WISE MENU</span><h1>{category}</h1><p>{filtered.length} {filtered.length===1?"item":"items"}</p></div>{pendingOrder&&<div className="wm-pending-mini"><span/> Pending Order <b>{pendingOrder.order_no}</b></div>}</div>{loading?<div className="wm-loading">Loading menu…</div>:filtered.length===0?<div className="wm-empty"><ShoppingBag size={30}/><b>No products found</b><span>Try another category or search.</span></div>:<section className="wm-grid">{filtered.map(p=><article className="wm-card" key={p.id}><button type="button" className="wm-media" onClick={()=>setProductOpen(p)} aria-label={`View ${p.name}`}>{p.video_url?<video src={p.video_url} autoPlay muted loop playsInline preload="metadata"/>:p.image_url?<img src={p.image_url} alt={p.name}/>:<div className="wm-placeholder">WISE</div>}<span className="wm-view-chip">View</span></button><div className="wm-card-body"><small>{p.category||"Menu"}</small><h2>{p.name}</h2><p>{p.description||""}</p><div className="wm-card-bottom"><strong>₱{Number(p.price).toFixed(2)}</strong><button type="button" onClick={()=>add(p)}><Plus size={17}/> Add to Cart</button></div></div></article>)}</section>}</section></div>
    ...
    {productOpen&&<div className="wm-product-overlay" onClick={()=>setProductOpen(null)}><section className="wm-product-modal" onClick={e=>e.stopPropagation()}><button className="wm-modal-close" type="button" onClick={()=>setProductOpen(null)}><X/></button><div className="wm-product-media">{productOpen.video_url?<video src={productOpen.video_url} autoPlay muted loop playsInline controls/>:productOpen.image_url?<img src={productOpen.image_url} alt={productOpen.name}/>:<div className="wm-placeholder">WISE</div>}</div><div className="wm-product-info"><small>{productOpen.category||"Menu"}</small><h2>{productOpen.name}</h2><strong>₱{Number(productOpen.price).toFixed(2)}</strong><p>{productOpen.description||"Quality service from WISE MENU."}</p><div className="wm-product-actions"><button type="button" className="wm-add-cart-large" onClick={()=>add(productOpen)}><ShoppingBag size={18}/> Add to Cart · ₱{Number(productOpen.price).toFixed(2)}</button></div></div></section></div>}
    {cartOpen&&<div className="wm-overlay"><aside className="wm-drawer"><header><div><small>WISE MENU</small><h2>My Order <span>({count} items)</span></h2></div><button type="button" onClick={()=>setCartOpen(false)}><X/></button></header><div className="wm-cart-list">{cart.map(i=><div className="wm-cart-row" key={i.product.id}><div><b>{i.product.name}</b><small>₱{Number(i.product.price).toFixed(2)} each</small></div><div className="wm-qty"><button type="button" onClick={()=>change(i.product.id,-1)}><Minus size={15}/></button><b>{i.quantity}</b><button type="button" onClick={()=>change(i.product.id,1)}><Plus size={15}/></button></div></div>)}</div><div className="wm-checkout"><div className="wm-section-label">HOW WOULD YOU LIKE TO RECEIVE YOUR ORDER?</div><div className="wm-order-type"><button type="button" className={orderType==="dine_in"?"active":""} onClick={()=>setOrderType("dine_in")}><Utensils size={20}/><b>Dine In</b><small>Enjoy here</small></button><button type="button" className={orderType==="take_out"?"active":""} onClick={()=>setOrderType("take_out")}><ShoppingBag size={20}/><b>Take Out</b><small>Take it with you</small></button></div><div className="wm-customer-confirm"><span>ORDERING AS</span><b>{name}</b><small>{email}</small></div><textarea value={notes} onChange={e=>setNotes(e.target.value)} placeholder="Special instructions (optional)"/>{submitError&&<div className="wm-submit-error" role="alert">{submitError}</div>}<div className="wm-total"><span>Total</span><b>₱{total.toFixed(2)}</b></div><button type="button" className="wm-submit-order" disabled={sending||!cart.length} onClick={submit}>{sending?"Submitting…":"Done"}<ChevronRight size={18}/></button></div></aside></div>}
  </main>;
}
