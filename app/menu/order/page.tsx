...
export default function WiseMenuOrder() {
  ...
  const add=(p:Product,quantity=1)=>{setSubmitError("");setCart(c=>{const x=c.find(i=>i.product.id===p.id);return x?c.map(i=>i.product.id===p.id?{...i,quantity:i.quantity+quantity}:i):[...c,{product:p,quantity}]});setProductOpen(null);setSelectedQty(1)};
