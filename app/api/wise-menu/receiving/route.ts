...
export async function GET(req:NextRequest){
 try{
  ...
  const {data,error}=await admin.from("wise_menu_orders").select("id,order_no,customer_name,customer_email,notes,status,order_type,total,subtotal,created_at,updated_at,wise_menu_order_items(product_id,product_name,quantity,unit_price,line_total,options)").in("status",["new","accepted"]).order("created_at",{ascending:false});
...
export async function PATCH(req:NextRequest){
 ...
 }catch(e:any){const message=e.message||"Unable to send order to Current Sale";const status=message==="Authentication required"?401:message==="Not authorized"?403:400;return NextResponse.json({error:message},{status});}
}
