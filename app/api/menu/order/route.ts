...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    if (!tableCode || items.length < 1 || items.length > 50) return NextResponse.json({ error: "Please select at least one item." }, { status: 400 });
    const client = createClient(url, anonKey, { auth: { persistSession: false, autoRefreshToken: false } });
    const { data, error } = await client.rpc("create_wise_menu_order", { p_table_code: tableCode, p_customer_name: customerName, p_notes: notes, p_items: items });
