...
export default function ReceivedFileJobPage() {
  ...
  const openCustomerUpdates = () => {
    if (!job) return;
    window.location.href = `/received-files/${job.id}/updates`;
  };

  if (loading) {
    return <div className="app-shell received-shell"><Sidebar /><main className="received-main"><div className="job-loading"><LoaderCircle className="spin" size={30} /><b>Loading file job…</b></div></main></div>;
  }
  ...
  return (
    <div className="app-shell received-shell">
      ...
      <main className="received-main">
        ...
        <section className="job-grid">
          ...
          <article className="job-card workflow-card">
            ...
            <div className="workflow-buttons">
              {statuses.map((status) => <button key={status} className={job.status === status ? "active" : ""} disabled={updating} onClick={() => updateStatus(status)}>{status}</button>)}
            </div>
            <button className="customer-updates-btn" onClick={openCustomerUpdates}>
              <MessageSquare size={18} /> CUSTOMER UPDATES
            </button>
            <span className="customer-updates-hint">Send, save and review customer notifications.</span>
        ...
        <section className="next-phase-card"><CheckCircle2 size={22} /><div><span className="mini-label">READY FOR PAYMENT</span><b>{printableCount} printable file{printableCount === 1 ? "" : "s"} ready to send to the POS.</b><p>The calculated print items will be added to the POS cart under <strong>{job.customer_name}</strong>, where staff can collect payment and print the customer receipt.</p></div><div className="next-total"><small>Job total</small><strong>₱{estimatedTotal.toFixed(2)}</strong></div><button className="send-pos-btn" disabled={sendingToPOS || printableCount === 0 || estimatedTotal <= 0} onClick={sendToPOS}><ShoppingCart size={18} /> {sendingToPOS ? "ADDING..." : "ADD JOB TO POS"}</button></section>
      </main>
