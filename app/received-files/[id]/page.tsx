...
function defaultSetup(): FileSetup {
  return { mode: "PRINT", paperSize: "A4", paperQuality: "Standard", colorMode: "Black & White", inkCoverage: "Normal", sides: "Single-sided", copies: 1, pages: 1 };
}
...
export default function ReceivedFileJobPage() {
  ...
  const loadJob = useCallback(async () => {
    ...
    if (loadError) setError(loadError.message || "Unable to load this file job.");
      ...
      (nextJob.received_file_items ?? []).forEach((file) => { nextSetups[file.id] = defaultSetup(); });
  ...
  const updateStatus = async (status: string) => {
    ...
    const { error: updateError } = await supabase.from("received_file_jobs").update({ status }).eq("id", job.id);
  ...
  const openFile = async (file: FileItem, download = false) => {
    const { data, error: urlError } = await supabase.storage.from("received-files")
      .createSignedUrl(file.storage_path, 60 * 15, download ? { download: file.original_name } : undefined);
    if (urlError || !data?.signedUrl) { setError(urlError?.message || "Unable to open this file."); return; }
    window.open(data.signedUrl, "_blank", "noopener,noreferrer");
  };

  const updateSetup = (fileId: string, patch: Partial<FileSetup>) => {
    setSetups((current) => ({ ...current, [fileId]: { ...(current[fileId] ?? defaultSetup()), ...patch } }));
  };
  ...
  const totalSize = useMemo(() => (job?.received_file_items ?? []).reduce((sum, file) => sum + Number(file.size_bytes || 0), 0), [job]);
  const estimatedTotal = useMemo(() => Object.values(setups).reduce((sum, setup) => sum + estimateFile(setup), 0), [setups]);

  if (loading) return <div className="app-shell received-shell"><Sidebar /><main className="received-main"><div className="job-loading"><LoaderCircle className="spin" size={30} /><b>Loading file job…</b></div><style jsx global>{styles}</style></main></div>;
  ...
  return <div className="app-shell received-shell">
    <Sidebar />
    <main className="received-main">
      ...
      <section className="job-grid">
        <article className="job-card customer-card"><span className="mini-label">CUSTOMER DETAILS</span><div className="customer-row"><span className="detail-icon"><UserRound size={20} /></span><div><small>Customer Name</small><b>{job.customer_name}</b></div></div><div className="customer-row"><span className="detail-icon"><Phone size={20} /></span><div><small>Contact Number</small><b>{job.contact_number}</b></div></div></article>
        <article className="job-card workflow-card"><span className="mini-label">JOB WORKFLOW</span><h2>Update job status</h2><p>Move the submission through the file processing workflow.</p><div className="workflow-buttons">{statuses.map((status) => <button key={status} className={job.status === status ? "active" : ""} disabled={updating} onClick={() => updateStatus(status)}>{status.replaceAll("_", " ")}</button>)}</div>{error && <div className="job-error">{error}</div>}</article>
      </section>

      <section className="job-card files-card">
        <div className="files-head"><div><span className="mini-label">STEP 3 · FILE PROCESSING</span><h2>Review and configure submitted files</h2><p>{files.length} file{files.length === 1 ? "" : "s"} · {formatBytes(totalSize)} total size</p></div><div className="estimate-total"><small>Current estimated total</small><b>₱{estimatedTotal.toFixed(2)}</b></div></div>

        <div className="processing-note"><Settings2 size={19} /><span>Choose what happens to each file, then set its print specifications. This is a live setup preview; final pricing rules will be refined in the next phase.</span></div>
        ...
        <div className="processing-list">
          {files.map((file, index) => {
            ...
            return <article className={`processing-file ${isOpen ? "open" : ""}`} key={file.id}>
              <button className="processing-file-head" onClick={() => setActiveFileId(isOpen ? null : file.id)}>
                <span className="file-number">{index + 1}</span><span className="file-icon"><FileText size={20} /></span><span className="file-title"><b>{file.original_name}</b><small>{file.mime_type || "Unknown file type"} · {formatBytes(Number(file.size_bytes || 0))}</small></span><span className="file-estimate">{setup.mode === "PRINT" ? `₱${estimate.toFixed(2)} est.` : setup.mode === "EDIT" ? "Needs editing" : "Skipped"}</span><ChevronDown size={20} className={isOpen ? "chevron open" : "chevron"} />
              </button>

              {isOpen && <div className="processing-body">
                <div className="file-actions"><button onClick={() => openFile(file)}><ExternalLink size={17} /> Open</button><button onClick={() => openFile(file, true)}><Download size={17} /> Download</button></div>
                <div className="mode-switch"><button className={setup.mode === "PRINT" ? "selected" : ""} onClick={() => updateSetup(file.id, { mode: "PRINT" })}><Printer size={18} /> Print Directly</button><button className={setup.mode === "EDIT" ? "selected" : ""} onClick={() => updateSetup(file.id, { mode: "EDIT" })}><FileText size={18} /> Edit / Customize First</button><button className={setup.mode === "SKIP" ? "selected muted" : "muted"} onClick={() => updateSetup(file.id, { mode: "SKIP" })}>Skip File</button></div>

                {setup.mode === "PRINT" && <div className="setup-grid">
                ...
                {setup.mode === "SKIP" && <div className="skip-guide">This file will not be included in the print calculation or the future POS cart.</div>}
              </div>}
            </article>;
          })}
        </div>
      ...
      <section className="next-phase-card"><CheckCircle2 size={22} /><div><span className="mini-label">STEP 3 IN PROGRESS</span><b>Print setup is now ready for every submitted file.</b><p>Next, we will connect the real pricing rules and create the <strong>Add to Cart → POS</strong> workflow.</p></div><div className="next-total"><small>Current job estimate</small><strong>₱{estimatedTotal.toFixed(2)}</strong></div></section>
    </main>
