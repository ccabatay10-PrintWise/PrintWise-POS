...
export default function ReceivedFileJobPage(){
 ...
 const updateSetup=(id:string,patch:Partial<FileSetup>)=>setSetups(current=>{
  ...
  const manualChange=changes.some(key=>!["mode","sides"].includes(key));
  return {...current,[id]:{...currentSetup,...patch,...(manualChange?{smartPrice:undefined,smartApplied:false,smartComputation:undefined}:{})}};
 });
