  ...
  return (
    <main className="app-shell">
      ...
      {completedReceipt && <>
        ...
        <section className="thermal-receipt" aria-hidden="true"><div className="thermal-inner"><div className="thermal-brand">PRINTWISE</div><div className="thermal-subtitle">Printing & Customized Services</div><div className="thermal-rule" /><div className="thermal-meta"><div><span>ORDER</span><b>{completedReceipt.orderNo}</b></div><div><span>DATE</span><b>{new Date(completedReceipt.createdAt).toLocaleString()}</b></div><div><span>CUSTOMER</span><b>{completedReceipt.customer}</b></div><div><span>TRANSACTED BY</span><b>{completedReceipt.transactedBy}</b></div><div><span>PAYMENT</span><b>{completedReceipt.payment}</b></div></div><div className="thermal-rule dashed" /><div className="thermal-items"><div className="thermal-item-head"><span>ITEM</span><span>AMOUNT</span></div>{completedReceipt.items.map((item, index) => <div className="thermal-item" key={`${item.id}-${index}`}><div><b>{item.name}</b><small>{item.quantity} × ₱{item.price.toFixed(2)}</small></div><strong>₱{(item.price * item.quantity).toFixed(2)}</strong></div>)}</div><div className="thermal-rule dashed" /><div className="thermal-totals"><div><span>Total Price</span><b>₱{completedReceipt.subtotal.toFixed(2)}</b></div>{completedReceipt.discount > 0 && <div><span>Discount</span><b>-₱{completedReceipt.discount.toFixed(2)}</b></div>}<div className="thermal-grand"><span>AMOUNT DUE</span><b>₱{completedReceipt.total.toFixed(2)}</b></div><div><span>Amount Paid</span><b>₱{completedReceipt.amountPaid.toFixed(2)}</b></div>{completedReceipt.payment === "Cash" && <div><span>Change</span><b>₱{completedReceipt.change.toFixed(2)}</b></div>}</div><div className="thermal-rule" /><p className="thermal-thankyou">THANK YOU FOR CHOOSING PRINTWISE!</p><p className="thermal-footer">Please keep this receipt for your records.</p></div></section>
      </>}
    </main>
  );
}
