...
export default function POSPage() {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      try {
        ...
        setProducts((payload.products ?? []).map((p: any) => ({ ...p, price: Number(p.price) })));
      } catch (error: any) {
  ...
  return <main className="app-shell"><Sidebar/><section className="wise-pos-page">
    ...
    <div className="wise-layout">
      <section className="wise-catalog"><div className="wise-toolbar"><div className="wise-search"><Search size={18}/><input value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Search products or services..."/></div>{categories.length > 1 && <div className="wise-categories">{categories.map((c) => <button key={c} className={category === c ? "active" : ""} onClick={() => setCategory(c)}>{c}</button>)}</div>}</div>
      {message && <div className="wise-notice">{message}</div>}
      {productsLoading ? <div className="wise-empty"><ShoppingCart size={34}/><strong>Loading products & services...</strong><span>WISE POS is loading the items enabled for checkout.</span></div> : filtered.length ? <div className="wise-product-grid">{filtered.map((p) => <button className="wise-product" key={p.id} onClick={() => add(p)}>{p.image_url ? <img src={p.image_url} alt=""/> : <div className="wise-product-letter">{p.name.charAt(0).toUpperCase()}</div>}<div className="wise-product-info"><strong>{p.name}</strong><span>{p.category}</span><b>{money(p.price)}</b></div><span className="wise-add"><Plus size={16}/></span></button>)}</div> : <div className="wise-empty"><ShoppingCart size={34}/><strong>No products or services are enabled for POS</strong><span>Go to Products & Services and turn on “Show in POS” for the items you want to sell.</span></div>}</section>
