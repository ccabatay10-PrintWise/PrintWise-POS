...
if (!source.includes(newHero)) {
  if (!source.includes(oldHero)) throw new Error("PrintWise: Incoming File Job hero marker was not found.");
  source = source.replace(oldHero, newHero);
}

const filesStart = source.indexOf("  <section className=\"job-card files-card\">");
const mainEnd = filesStart >= 0 ? source.indexOf("  </main>", filesStart) : -1;
if (filesStart >= 0 && mainEnd >= 0) {
  source = source.slice(0, filesStart) + source.slice(mainEnd);
}
...
if (!source.includes(".incoming-file-job{")) {
  ...
  if (styleEnd < 0) throw new Error("PrintWise: Received Files styles marker was not found.");
  source = source.slice(0, styleEnd) + css + source.slice(styleEnd);
}

fs.writeFileSync(filePath, source, "utf8");
console.log("PrintWise: Incoming File Job consolidated successfully.");
